...
\begin{definition}{$\epsilon$-$\delta$-Stetigkeit}
	\begin{multicols}{2}
		...
		Oder kurz, falls gilt:
		\begin{equation*}
			f\left(K_\delta(x_0)\right)\subseteq K_\epsilon(f(x_0))
		\end{equation*}
	\columnbreak

	\vspace*{-3.5em}
	\begin{tikzpicture}
		\fill[fill=green!30!white, draw=none] (0,0.4) rectangle (4.6,1.6);
		\draw[green, line width=0.2mm] (0,0.4) to (4.6,0.4);
		\draw[green, line width=0.2mm] (0,1.6) to (4.6,1.6);
		...
		\draw[dashed] (fx0achse) -- (fx0);

		\draw[line width=0.5mm, scale=1, domain=0:2, smooth, variable=\x, blue] plot ({\x},{-0.1*(\x-2)*(\x-2)+1}) node[below right] {};

		\draw[line width=0.5mm, scale=1, domain=2:4.6, smooth, variable=\x, blue] plot ({\x},{0.1*(\x-2)*(\x-2)+1.8}) node[below right] {};
